...
upgradeChaincode() {
  ...
  set -x
  peer chaincode upgrade -o orderer.example.com:7050 --cafile $ORDERER_CA -C mychannel -n tracktrace -v 2.1 -c '{"Args":[""]}' -P "AND ('Org1MSP.peer','Org2MSP.peer','Org3MSP.peer')"
  res=$?
  set +x
  cat log.txt
